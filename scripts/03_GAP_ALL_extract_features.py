...
def obtain_features(model, x):
    ...
    if isinstance(model, ResNet) or isinstance(model, DenseNet):
        for name, module in model._modules.items():
            ...
            if "layer" in name or "trans" in name or "relu" in name:
...
def run():
    for model_name, model_info in models.items():
        ...
        for loader_name, loader in loaders.items():
            print(">>>", loader_name)
            path = "./features/gap_all/{}/{}".format(model_name, loader_name)
...
def run_attacks():
    ...
    for model_name, model_info in models.items():
        ...
        for method_name in method_folders:
            ...
            print(">>>", method_name)
            path = "./features/gap_all/{}/{}".format(model_name, method_name)
