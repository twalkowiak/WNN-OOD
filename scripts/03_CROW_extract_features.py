...
class CroW(torch.nn.Module):
    ...
    def forward(x):
        ...
        if fea.ndimension() == 4:
            ...
            c, w, h = fea.shape[1:]
            non_zeros = (fea != 0).float().sum(
                dim=(2, 3)) / 1.0 / (w * h) + 1e-6
            channel_weight = torch.log(
                non_zeros.sum(
                    dim=1,
                    keepdims=True) /
                non_zeros)
    ...
    if isinstance(model, WideResNet):
        for name, module in model._modules.items():
            ...
            if "block" in name:
                ...
                out_all = torch.cat(
                    (out_all, out), dim=1) if out_all is not None else out
...
def run():
    for model_name, model_info in models.items():
        ...
        for loader_name, loader in loaders.items():
            print(">>>", loader_name)
            path = "../features/crow/{}/{}".format(model_name, loader_name)
...
def run_attacks():
    ...
    for model_name, model_info in models.items():
        ...
        for method_name in method_folders:
            ...
            print(">>>", method_name)
            path = "../features/crow/{}/{}".format(model_name, method_name)
