...
def load_raw(fe, model, ood, data):
    path = "../feature100/" + fe + "/" + model + \
        "/scores/" + ood + "_" + data + ".npy"
    # if ood == "KNN" and model == "cifar10_densenet":
    #    path = "../../generete_features_new/features/"+fe+"/"+model+"/scores_knn/"+ood+"_"+data+".npy"
    x = np.load(path)
    return x
...
def test_one(fe, model, ood, ind, outd, rate):
    ...
    # print(fe, model, ood, outd)
    x_in = load_raw(fe, model, ood, ind)
...
def test_trans_le(model="cifar10_densenet", name="noise", name2="svhn", fes=[
                  "gap", "crow", "gap_all", "scda", "gmp"], oods=["Mahalanobis", "lof_euclidean", "MaxSoftmax"], rate=0.2):
    X_in = []
    X_out = []
    X_out2 = []
    for ood in oods:
        for fe in fes:
            X_in.append(load_raw(fe, model, ood, test_name))
            X_out.append(load_raw(fe, model, ood, name))
            X_out2.append(load_raw(fe, model, ood, name2))
    ...
    X_train = np.transpose(np.asarray(
        [scalers[i](x) for i, x in enumerate(np.transpose(X_train))]))
    regr = LogisticRegressionCV(random_state=np.random.randint(
    ...
    if name == name2:
        x_out = get_data_one(X_out2, test_ind_out)
    else:
        x_out = get_data_one(X_out2, np.asarray(range(len(X_out2[0]))))
...
def test_trans_nn(model="cifar10_densenet", name="noise", name2="svhn", fes=[
                  "gap", "crow", "gap_all", "scda", "gmp"], oods=["Mahalanobis", "lof_euclidean", "MaxSoftmax"], rate=0.2):
    X_in = []
    X_out = []
    X_out2 = []
...
def multi_trans_test(model, name, name2, rate):
    metrics = {}
    results = []

    for i in range(number_test):
        results.append(test_trans_nn(model, name, name2, ["gap", "crow", "gap_all", "scda", "gmp"], oods=[
                       "Mahalanobis", "lof_euclidean", "MaxSoftmax"], rate=rate))
    results = np.asarray(results)
    resp = [np.mean(results, axis=0).tolist(),
            np.std(results, axis=0).tolist()]
    ...
    results = []
    for i in range(number_test):
        results.append(test_one("gap", model, "KNN", test_name, name2, rate))
    results = np.asarray(results)
    resp = [np.mean(results, axis=0).tolist(),
            np.std(results, axis=0).tolist()]
    ...
    for ood in ["Mahalanobis", "lof_euclidean", "MaxSoftmax"]:
        results = []
        for i in range(number_test):
            results.append(test_one("gap", model, ood, test_name, name2, rate))
        results = np.asarray(results)
        resp = [np.mean(results, axis=0).tolist(),
                np.std(results, axis=0).tolist()]
        print("  " + ood, resp)
...
def attacks_cw(results, datas, rate):
    # models = ['ResNet','AlexNet','ShuffleNetV2','WideResNet','MobileNet2','VGG2']
    for model in models:
...
def test_linear():
    ...
    test_name = "test"
    r = 0.2
    datas = ["imagenet_o", "textures", "tiny_imagenet", "cifar100", "svhn"]
    # models = ["cifar10_densenet", "cifar10_resnet", "cifar10_wrn"]
    models = [
        'ResNet',
        'AlexNet',
        'ShuffleNetV2',
    ...
    for name in datas:
        ...
        for model in models:
            print("  Model: ", model)
            try:
                resp = multi_trans_test(model, name, name, rate=r)
...
def test_ood():
    ...
    test_name = "test"
    r = 0.2
    datas = ["imagenet_o", "textures", "tiny_imagenet", "cifar10", "svhn"]
    # models = ["cifar10_densenet", "cifar10_resnet", "cifar10_wrn"]
    models = [
        'ResNet',
        'AlexNet',
    ...
    for name in datas:
        ...
        for name2 in datas:
            results[name][name2] = {}
            print(r, menu, ": ", name, name2)
            for model in models:
                print("  Model: ", model)
                try:
                    resp = multi_trans_test(model, name, name2, rate=r)
                    results[name][name2][model] = resp
                except FileNotFoundError as e:
                    print("results not avaliable " + str(e))
    with open("../results/" + path + ".json", "wt") as f:
        json.dump(results, f)
...
def test_attacks_knn():
    ...
    datas = [
        ...
        "Square_hard"]
    models = ["cifar10_resnet"]
    # models = ['ResNet','AlexNet','ShuffleNetV2','WideResNet','MobileNetV2','VGG16']
    # models = ['MobileNetV2','VGG16']
    path = "attacks_knn"
